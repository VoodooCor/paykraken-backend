...
async function getMerchantTokenAccount() {
  if (MERCHANT_TOKEN_ACCOUNT) {
    return MERCHANT_TOKEN_ACCOUNT;
  }

  if (!MERCHANT_WALLET) {
    throw new Error('MERCHANT_WALLET is not set');
  }

  if (!BLKR_MINT) {
    throw new Error('BLKR_MINT is not set');
  }
  ...
  if (!list.length) {
    throw new Error(
      'Associated token account for BLKR not found for MERCHANT_WALLET. Create ATA or set MERCHANT_TOKEN_ACCOUNT.'
    );
  }
